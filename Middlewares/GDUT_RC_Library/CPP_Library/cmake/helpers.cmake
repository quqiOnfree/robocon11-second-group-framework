function(etl_determine_version_with_file VER_FILE_NAME)
    file(READ ${VER_FILE_NAME} GDUT_VERSION_RAW)
    # Remove trailing whitespaces and/or newline
    string(STRIP ${GDUT_VERSION_RAW} GDUT_VERSION)
    set(GDUT_VERSION ${GDUT_VERSION} CACHE STRING
        "ETL version determined from version.txt" FORCE
    )
    message(STATUS "${MSG_PREFIX} Determined ETL version ${GDUT_VERSION} from version.txt file")
endfunction()
...
function(etl_determine_version_with_git)
    ...
    set(GDUT_VERSION "${VERSION_MAJOR}.${VERSION_MINOR}.${VERSION_PATCH}")

    set(GDUT_VERSION ${GDUT_VERSION} CACHE STRING
        "ETL version determined with git describe" FORCE
    )
    message(STATUS "${MSG_PREFIX} Determined ETL version ${GDUT_VERSION} from the git tag")
endfunction()
