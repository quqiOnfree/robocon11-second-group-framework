...
base_can_proxy::~base_can_proxy() noexcept = default;
  ...
  for (size_t bus = 0; bus < bus_count; ++bus) {
    for (size_t i = 0; i < instances_count[bus]; ++i) {
      if (instances[bus][i] == this) {
        // 将后续元素前移覆盖当前位置
        size_t j = i;
        for (; j < instances_count[bus] - 1 && instances[bus][j + 1] != nullptr;
             ++j) {
...
bool base_can_proxy::transmit(const uint8_t data[8]) {
  ...
  uint32_t mailbox{0};
  // 将帧加入发送队列，硬件会自动仲裁并发送
  auto status = HAL_CAN_AddTxMessage(&m_hcan, &m_tx_header, data, &mailbox);
  if (status != HAL_OK) {
