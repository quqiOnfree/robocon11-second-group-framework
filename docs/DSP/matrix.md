...
| `make_rotate(axis, angle)` | 绕轴 `axis` 旋转 `angle` 角度（Rodrigues 公式），函数内部自动对 `axis` 归一化 |
...
- **`make_rotate` 角度单位为“度”**：函数内部会将 `angle` 转换为弧度后再计算三角函数（即 $\theta_{rad}=\theta_{deg}\cdot\pi/180$）。
